...
const TemperatureAndDetails = ({ data }) => {
  ...
  return (
    <div className="">
      <div className="flex">
      {<img  className="my-2" src={weatherData.icon} width={70} />}
        <p className={`${darkMode ? "text-gray-200" : "text-neutral-900"} text-2xl md:text-3xl pt-6 mt-4 md:pt-2 mx-3`}>
          {data.main.temp.toFixed()}°C
        </p>
      </div>
      <div className="flex py-2">
        <p className="font-medium  text-2xl pt-2 mx-2 md:text-2xl text-blue-400">
          {weatherData?.translation}
        </p>
      </div>

      <div className={`flex flex-col md:flex-row justify-between mr-4 gap-2 py-3 ${darkMode ? "text-gray-200" : "text-neutral-900"}`}>
        <div>
